...
/*
 * ILI9341 SPI framebuffer driver for a 320x240 RGB565 panel.
 */
...
/**
 * struct ili9341_par - per-device framebuffer state
 * @spi: SPI device for the ILI9341 panel on chip select 0.
 * @info: fbdev information allocated for this device.
 * @fbdefio: deferred-I/O configuration used to batch display refreshes.
 * @dc_gpio: data/command GPIO, active high for data.
 * @reset_gpio: display reset GPIO.
 * @led_gpio: display backlight GPIO.
 * @spi_tx_buf: temporary transmit buffer used so framebuffer memory is not
 * modified in place while converting pixels for SPI transfer.
 * @rotation: panel rotation read from device tree, defaults to landscape.
 * @fps: deferred-I/O refresh rate read from device tree.
 * @frame_count: optional refresh counter exposed through sysfs.
 */
struct ili9341_par {
...
static int ili9341_init_display(struct ili9341_par *par)
{
	...
	/* This panel requires BGR color order; fbdev remains RGB565. */
...
static inline void convert_pixel_for_panel(const u8 *src, u8 *dst)
{
	/* fbdev stores RGB565 little-endian; ILI9341 SPI sends high byte first. */
	dst[0] = src[1];
	dst[1] = src[0];
}
...
static void ili9341_deferred_io(struct fb_info *info, struct list_head *pagelist)
{
	...
	/* Convert and transmit one bounded SPI chunk at a time. */
	for (i = 0; i < VMEM_SIZE; i += SPI_CHUNK_SIZE) {
		size_t len;
		int j;

		len = min_t(size_t, VMEM_SIZE - i, SPI_CHUNK_SIZE);

		for (j = 0; j < len; j += 2)
			convert_pixel_for_panel(&src[i + j], &dst[j]);

		spi_write(par->spi, dst, len);
	}
}

static const struct fb_ops ili9341_ops = {
	.owner        = THIS_MODULE,
	.fb_read      = fb_sys_read,
...
static int ili9341_probe(struct spi_device *spi)
{
	...
	dev_info(&spi->dev, "ILI9341 framebuffer driver probe\n");
	...
	rotation = 90;
	if (!of_property_read_u32(spi->dev.of_node, "rotation", &rotation) &&
	...
	fps = DEFAULT_FPS;
	if (!of_property_read_u32(spi->dev.of_node, "fps", &fps) &&
	...
	par->fbdefio.delay = max_t(unsigned long, 1, HZ / par->fps);
	...
	dev_info(&spi->dev, "ILI9341 framebuffer driver loaded (rotation=%u, fps=%u)\n",
		 par->rotation, par->fps);
