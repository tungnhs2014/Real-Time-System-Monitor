 ...
 *
 * The controller is connected over SPI with an active-low PENIRQ GPIO. The IRQ
 * schedules delayed work, and the worker polls coordinates while the pen
 * remains down.
 *
 * Author: TungNHS
 */
...
/* Performance tuning */
#define POLL_INTERVAL_MS    5    /* Poll interval while pen is down */
#define SETTLE_DELAY_US     100  /* ADC settle time */
...
static void xpt2046_remove(struct spi_device *spi)
#else
